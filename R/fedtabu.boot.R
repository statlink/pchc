fedtabu.boot <- function(x, method = "pearson", alpha = 0.05, ini.stat = NULL,
                         R = NULL, tabu = 10, score = "bic-g", blacklist = NULL,
                         whitelist = NULL, B = 200, ncores = 1) {

  mod <- pchc::fedtabu(x, method = method, alpha = alpha, ini.stat = ini.stat, R = R, tabu = tabu,
  ...
  if ( ncores == 1 ) {
    ...
    for (i in 1:B) {
      ...
      gb <- pchc::fedtabu(x[id, ], method = method, alpha = alpha, tabu = tabu,
                          score = score, blacklist = blacklist, whitelist = whitelist)
