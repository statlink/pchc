pchc.boot <- function(x, method = "pearson", alpha = 0.05, ini.stat = NULL,
                      R = NULL, restart = 10, score = "bic-g", blacklist = NULL,
                      whitelist = NULL, B = 200, ncores = 1) {

  mod <- pchc::pchc(x, method = method, alpha = alpha, ini.stat = ini.stat, R = R, restart = restart,
  ...
  if ( ncores == 1 ) {
    ...
    for (i in 1:B) {
      ...
      gb <- pchc::pchc(x[id, ], method = method, alpha = alpha, restart = restart,
                       score = score, blacklist = blacklist, whitelist = whitelist)
